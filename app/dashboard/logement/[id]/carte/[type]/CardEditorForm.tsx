...
function CardFields({ type, data }: { type: CardType; data: Record<string, string> }) {
  switch (type) {
    case 'wifi':
      return (
        <>
          <Field label="Nom du réseau (SSID)" name="ssid" defaultValue={data.ssid} placeholder="MonWiFi" required />
          <Field label="Mot de passe" name="password" defaultValue={data.password} placeholder="••••••••" required />
        </>
      )
    case 'checkin':
      return (
        <>
          <TextareaField label="Instructions d'arrivée" name="instructions" defaultValue={data.instructions} placeholder="La clé se trouve dans la boîte à clés au-dessus de la porte." required />
          <Field label="Code d'accès" name="code" defaultValue={data.code} placeholder="1234" />
          <Field label="Localisation de la clé" name="keyLocation" defaultValue={data.keyLocation} placeholder="Boîte à clés, concierge…" />
          <Field label="Heure de check-in" name="checkInTime" defaultValue={data.checkInTime} placeholder="À partir de 15h00" />
        </>
      )
    case 'checkout':
      return (
        <>
          <TextareaField label="Instructions de départ" name="instructions" defaultValue={data.instructions} placeholder="Laissez les clés sur la table, fermez bien la porte." required />
          <Field label="Heure de check-out" name="checkOutTime" defaultValue={data.checkOutTime} placeholder="Avant 11h00" />
        </>
      )
    case 'rules':
      return (
        <TextareaField label="Règles du logement" name="rules" defaultValue={data.rules} rows={8}
          placeholder={"- Pas de bruit après 22h\n- Non fumeur\n- Pas d'animaux\n- Respectez les voisins"} required />
      )
    case 'contact':
      return (
        <>
          <Field label="Prénom / nom" name="name" defaultValue={data.name} placeholder="Marie Dupont" required />
          <Field label="Téléphone" name="phone" defaultValue={data.phone} placeholder="+33 6 12 34 56 78" type="tel" />
          <Field label="Email" name="email" defaultValue={data.email} placeholder="marie@example.com" type="email" />
          <TextareaField label="Note" name="notes" defaultValue={data.notes} placeholder="N'hésitez pas à m'appeler en cas de problème." rows={3} />
        </>
      )
    case 'transport':
      return (
        <TextareaField label="Comment accéder au logement" name="instructions" defaultValue={data.instructions} rows={7}
          placeholder="Métro ligne 5, station Oberkampf (10 min à pied). Bus 56 arrêt Voltaire." required />
      )
    case 'tips':
      return (
        <TextareaField label="Recommandations locales" name="recommendations" defaultValue={data.recommendations} rows={8}
...
export function CardEditorForm({ propertyId, type, propertyName, existingData }: CardEditorFormProps) {
  ...
  return (
    <div className="max-w-md">
      ...
      {/* Card type badge */}
      <div className="inline-flex items-center gap-2 bg-accent/[0.08] border border-accent/[0.15] rounded-lg px-3 py-1.5 mb-5 text-accent">
