...
export function CoverPhotoUpload({ propertyId, current }: { propertyId: string; current: string | null }) {
  ...
  return (
    <div className="bg-surface border border-white/[0.07] rounded-2xl overflow-hidden mb-8">
      {/* Preview */}
      {preview ? (
        <div className="relative h-36 w-full">
          ...
          <div className="absolute bottom-3 right-3 flex gap-2">
            <label className={`inline-flex items-center gap-1.5 bg-black/50 border border-white/20 rounded-lg px-3 py-1.5 text-white/70 hover:text-white text-xs font-medium transition-colors cursor-pointer ${busy ? 'opacity-40 pointer-events-none' : ''}`}>
            ...
            <button type="button" onClick={remove} disabled={busy}
              className="inline-flex items-center gap-1.5 bg-black/50 border border-red-500/30 rounded-lg px-3 py-1.5 text-red-400/80 hover:text-red-400 text-xs font-medium transition-colors disabled:opacity-40">
        ...
        <label className={`flex flex-col items-center justify-center gap-3 h-28 cursor-pointer group transition-colors ${busy ? 'opacity-40 pointer-events-none' : 'hover:bg-white/[0.02]'}`}>
          <div className="w-9 h-9 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center group-hover:border-white/[0.14] transition-colors">
            {busy ? (
