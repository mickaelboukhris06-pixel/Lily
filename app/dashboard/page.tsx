    ...
    <div>
      ...
      {properties.length === 0 ? (
        <div className="border border-dashed border-white/[0.08] rounded-2xl p-20 text-center">
          <div className="w-12 h-12 rounded-2xl bg-white/[0.04] border border-white/[0.07] flex items-center justify-center mx-auto mb-5">
          ...
          <Link
            ...
            className="bg-accent text-white font-semibold px-5 py-2.5 rounded-xl hover:bg-[#8880ff] transition-all duration-200 text-sm inline-flex items-center gap-2 shadow-[0_0_16px_rgba(121,113,255,0.2)]"
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M12 5v14M5 12h14" />
            </svg>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {properties.map((property) => (
            <Link
              ...
              className="bg-surface border border-white/[0.07] rounded-2xl p-6 hover:border-white/[0.13] hover:bg-surface-hi transition-all duration-200 group"
            >
              ...
              <div className="flex items-center gap-2 mt-4">
                <span className="text-[11px] text-white/25 bg-white/[0.05] px-2.5 py-1 rounded-full">
                  {property.cards.length} carte{property.cards.length !== 1 ? 's' : ''} actives
                </span>
                {property.guestToken && (
                  <span className="text-[11px] text-accent bg-accent/[0.08] border border-accent/20 px-2.5 py-1 rounded-full">
                    Lien actif
                  </span>
                )}
              </div>
